...
def view1():
    if (request.method == 'POST'):
        ...
        response = requests.request("POST", url, cert=(cert, key), headers=headers, auth=(user_id, password), data=res,
                                    timeout=timeout)
        stores = response.json()["merchantLocatorServiceResponse"]["response"]
        infoList =[]
        for store in stores:
            store_info = store["responseValues"]
            infoList.append(store_info)
        return render_template('merchant_list.html',data = infoList)
